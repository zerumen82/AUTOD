...
def faceswap_tab():
    ...
    with gr.Tab("🎭 Cambio de Cara"):
        with gr.Row(variant='panel'):
            with gr.Column(scale=2):
                with gr.Row():
                    with gr.Column(min_width=160):
                        input_faces = gr.Gallery(label="Origen", allow_preview=False, preview=False, height=300,
                                                 object_fit="scale-down", columns=8)
                        with gr.Accordion(label="Enmascaramiento Avanzado", open=False):
                            chk_showmaskoffsets = gr.Checkbox(label="Mostrar superposición de máscara en vista previa",
                                                              value=False,
                                                              interactive=True)
                            mask_top = gr.Slider(0, 1.0, value=0, label="Cara compensada superior", step=0.01,
                                                 interactive=True)
                            mask_bottom = gr.Slider(0, 1.0, value=0, label="Cara inferior desplazada", step=0.01,
                                                    interactive=True)
                            mask_left = gr.Slider(0, 1.0, value=0, label="Cara desplazada izquierda", step=0.01,
                                                  interactive=True)
                            mask_right = gr.Slider(0, 1.0, value=0, label="Cara desplazada derecha", step=0.01,
                                                   interactive=True)
                            mask_erosion = gr.Slider(1.0, 3.0, value=1.0, label="Iteraciones de erosión", step=1.00,
                                                     interactive=True)
                            mask_blur = gr.Slider(10.0, 50.0, value=20.0, label="Tamaño de desenfoque", step=1.00,
                                                  interactive=True)
                            bt_toggle_masking = gr.Button("Toggle manual masking", variant='secondary', size='sm')
                            selected_mask_engine = gr.Dropdown(["None", "Clip2Seg", "DFL XSeg"], value="None",
                                                               label="Motor de enmascaramiento")
                            clip_text = gr.Textbox(label="Lista de objetos para enmascarar y restaurar",
                                                   value="cup,hands,hair,banana", interactive=False)
                    ...
                    with gr.Column(min_width=160):
                        target_faces = gr.Gallery(label="Objetivo", allow_preview=False, preview=False, height=300,
                                                  object_fit="scale-down", columns=8)
                ...
                with gr.Row(variant='panel'):
                    ...
                    bt_destfiles = gr.Files(label='Archivos Destino', file_count="multiple",
                                            file_types=["image", "video"], elem_id='filelist', height=300)
                with gr.Row(variant='panel'):
                    gr.Markdown('')
                    forced_fps = gr.Slider(minimum=0, maximum=120, value=0, label="Video FPS",
                                           info='Overrides detected fps if not 0', step=1.0, interactive=True,
                                           container=True)
            ...
            with gr.Column(scale=2):
                ...
                previewimage = gr.Image(label="Previsualización d la Imagen", height=576, interactive=False,
                                        visible=True)
                maskimage = gr.ImageEditor(label="Imagen de máscara manual", sources=["clipboard"], transforms="",
                                           type="numpy",
                                           brush=gr.Brush(color_mode="fixed", colors=["rgba(255, 255, 255, 1"]),
                                           interactive=True, visible=False)
                with gr.Row(variant='panel'):
                    fake_preview = gr.Checkbox(label="Frames de intercambio", value=False)
        ...
        with gr.Row(visible=False) as dynamic_face_selection:
            with gr.Column(scale=2):
                face_selection = gr.Gallery(label="Caras detectadas", allow_preview=False, preview=False, height=256,
                                            object_fit="cover", columns=8)
            with gr.Column():
        ...
        with gr.Row(variant='panel'):
            with gr.Column(scale=1):
                selected_face_detection = gr.Dropdown(
                    ["First found", "All female", "All male", "All faces", "Selected face"], value="Selected face",
                    label="Especificar la selección de caras para intercambiar")
            with gr.Column(scale=1):
                ui.globals.ui_selected_enhancer = gr.Dropdown(
                    ["None", "Codeformer", "DMDNet", "GFPGAN", "GPEN", "Restoreformer++"], value="GFPGAN",
                    label="Seleccionar post-procesado")

        with gr.Row(variant='panel'):
            with gr.Column(scale=1):
                max_face_distance = gr.Slider(0.01, 1.0, value=0.04, label="Umbral máximo de similitud facial",
                                              info="0.0 = idéntica 1.0 = sin similitud")
            with gr.Column(scale=1):
                num_swap_steps = gr.Slider(1, 5, value=3, step=1.0, label="Número de pasos de intercambio",
                                           info="Más pasos pueden aumentar la semejanza")
            with gr.Column(scale=2):
                ui.globals.ui_blend_ratio = gr.Slider(0.0, 1.0, value=0.90,
                                                      label="Relación de fusión de imagen original/mejorada",
                                                      info="Sólo se utiliza con posprocesamiento activo")

        with gr.Row(variant='panel'):
            with gr.Column(scale=1):
                video_swapping_method = gr.Dropdown(["Extract Frames to media", "In-Memory processing"],
                                                    value="In-Memory processing",
                                                    label="Seleccione el método de procesamiento de video",
                                                    interactive=True)
                no_face_action = gr.Dropdown(choices=no_face_choices, value=no_face_choices[0],
                                             label="Acción si no se detecta ningún rostro", interactive=True)
                vr_mode = gr.Checkbox(label="Modo VR", value=False)
            with gr.Column(scale=1):
                with gr.Group():
                    autorotate = gr.Checkbox(label="Rotación automática de caras horizontales", value=True)
                    roop.globals.skip_audio = gr.Checkbox(label="Saltar audio", value=False)
                    roop.globals.keep_frames = gr.Checkbox(
                        label="Mantener fotogramas (relevante sólo al extraer fotogramas)",
                        value=False)
                    roop.globals.wait_after_extraction = gr.Checkbox(
                        label="Espere a que el usuario presione la tecla antes de crear el video.", value=False)
        ...
        with gr.Row(variant='panel'):
            ...
            with gr.Column():
                ...
                resultvideo = gr.Video(label='Video Final', interactive=False, visible=False)

    previewinputs = [preview_frame_num, bt_destfiles, fake_preview, ui.globals.ui_selected_enhancer,
                     selected_face_detection,
                     max_face_distance, ui.globals.ui_blend_ratio, selected_mask_engine, clip_text, no_face_action,
                     vr_mode, autorotate, maskimage, chk_showmaskoffsets, num_swap_steps]
    previewoutputs = [previewimage, maskimage, preview_frame_num]
    input_faces.select(on_select_input_face, None, None).then(fn=on_preview_frame_changed, inputs=previewinputs,
                                                              outputs=previewoutputs)
    bt_remove_selected_input_face.click(fn=remove_selected_input_face, outputs=[input_faces])
    bt_srcfiles.change(fn=on_srcfile_changed, show_progress='full', inputs=bt_srcfiles,
                       outputs=[dynamic_face_selection, face_selection, input_faces])
    ...
    selected_mask_engine.change(fn=on_mask_engine_changed, inputs=[selected_mask_engine], outputs=[clip_text],
    ...
    bt_destfiles.change(fn=on_destfiles_changed, inputs=[bt_destfiles], outputs=[preview_frame_num, text_frame_clip],
                        show_progress='hidden').then(fn=on_preview_frame_changed, inputs=previewinputs,
                                                     outputs=previewoutputs, show_progress='hidden')
    bt_destfiles.select(fn=on_destfiles_selected, outputs=[preview_frame_num, text_frame_clip, forced_fps],
                        show_progress='hidden').then(fn=on_preview_frame_changed, inputs=previewinputs,
                                                     outputs=previewoutputs, show_progress='hidden')
    ...
    bt_preview_mask.click(fn=on_preview_mask, inputs=[preview_frame_num, bt_destfiles, clip_text, selected_mask_engine],
                          outputs=[previewimage])

    start_event = bt_start.click(fn=start_swap,
                                 inputs=[ui.globals.ui_selected_enhancer, selected_face_detection,
                                         roop.globals.keep_frames, roop.globals.wait_after_extraction,
                                         ...
                                         vr_mode, autorotate, num_swap_steps, maskimage],
    ...
    preview_frame_num.release(fn=on_preview_frame_changed, inputs=previewinputs, outputs=previewoutputs,
                              show_progress='hidden', )
    bt_use_face_from_preview.click(fn=on_use_face_from_selected, show_progress='full',
                                   inputs=[bt_destfiles, preview_frame_num],
                                   outputs=[dynamic_face_selection, face_selection, target_faces,
                                            selected_face_detection])
...
def on_add_local_folder(folder):
    ...
    if files is None:
        gr.Warning("¡Carpeta vacía o carpeta no encontrada!")
    return files
...
def on_srcfile_changed(srcfiles, progress=gr.Progress()):
    ...
    for f in srcfiles:
        ...
        if source_path.lower().endswith('fsz'):
            progress(0, desc="Recuperar caras del archivo Faceset")
            ...
            for file in os.listdir(unzipfolder):
                if file.endswith(".png"):
                    ...
                    progress(0, desc="Extrayendo faceset")
            ...
            progress(0, desc="Recuperar caras de la imagen")
            roop.globals.source_path = source_path
            from roop.face_util import extract_face_images
            SELECTION_FACES_DATA = extract_face_images(roop.globals.source_path, (False, 0))
            progress(0.5, desc="Recuperar caras de la imagen")
            for f in SELECTION_FACES_DATA:
                face_set = FaceSet()
                face = f[0]
                face.mask_offsets = (0, 0, 0, 0, 1, 20)
...
def on_preview_frame_changed(frame_num, files, fake_preview, enhancer, detection, face_distance, blend_ratio,
                             selected_mask_engine, clip_text, no_face_action, vr_mode, auto_rotate, maskimage,
                             show_face_area, num_steps):
    ...
    if not fake_preview or len(roop.globals.INPUT_FACESETS) < 1:
        return gr.Image(value=util.convert_to_gradio(current_frame), visible=True), gr.ImageEditor(
            visible=False), gr.Slider(info=timeinfo)
    ...
    options = ProcessOptions(get_processing_plugins(mask_engine), roop.globals.distance_threshold,
...
def on_toggle_masking(previewimage, mask):
    ...
    if manual_masking:
        ...
        return gr.Image(visible=False), gr.ImageEditor(
            value={"background": previewimage, "layers": layers, "composite": None}, visible=True)
...
def start_swap(enhancer, detection, keep_frames, wait_after_extraction, skip_audio, face_distance, blend_ratio,
               selected_mask_engine, clip_text, processing_method, no_face_action, vr_mode, autorotate, num_swap_steps,
               imagemask, progress=gr.Progress()):
    ...
    if not util.is_installed("ffmpeg"):
        msg = "¡ffmpeg no está instalado! No es posible el procesamiento de vídeo."
        gr.Warning(msg)
    ...
    batch_process_regular(list_files_process, mask_engine, clip_text, processing_method == "In-Memory processing",
                          imagemask, num_swap_steps, progress, SELECTED_INPUT_FACE_INDEX)
    is_processing = False
...
def on_destfiles_changed(destfiles):
    ...
    if total_frames > 1:
        return gr.Slider(value=1, maximum=total_frames, info='0:00:00'), gen_processing_text(
...
def on_destfiles_selected(evt: gr.SelectData):
    ...
    if total_frames > 1:
        return gr.Slider(value=list_files_process[idx].startframe, maximum=total_frames,
                         info='0:00:00'), gen_processing_text(list_files_process[idx].startframe,
                                                              list_files_process[idx].endframe), fps
