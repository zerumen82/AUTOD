...
def extras_tab():
    ...
    with gr.Tab("🎉 Extras"):
        with gr.Row():
            files_to_process = gr.Files(label='Archivos a procesar', file_count="multiple",
        ...
        with gr.Row(variant='panel'):
            with gr.Accordion(label="Video/GIF", open=False):
                with gr.Row(variant='panel'):
                    with gr.Column():
                        gr.Markdown("""
                                    # Editor de video del pobre
                                    La recodificación utiliza su configuración desde la pestaña Configuración.
    """)
                    with gr.Column():
                    ...
                    with gr.Column():
                        extras_chk_encode = gr.Checkbox(
                            label='Volver a codificar vídeos (necesario para vídeos con diferentes códecs)', value=False)
                ...
                with gr.Row(variant='panel'):
                    with gr.Column():
                        gr.Markdown("""
                                    # Crear video/gif a partir de imágenes
    """)
                    with gr.Column():
                        extras_fps = gr.Slider(minimum=0, maximum=120, value=30, label="Video FPS", step=1.0,
                                               interactive=True)
                        extras_images_folder = gr.Textbox(show_label=False, placeholder="/content/", interactive=True)
                    with gr.Column():
                        extras_chk_creategif = gr.Checkbox(label='Crear GIF a partir de vídeo', value=False)
                        extras_create_video = gr.Button("Create")
        with gr.Row(variant='panel'):
            with gr.Accordion(label="Procesamiento de fotograma completo", open=True):
                with gr.Row(variant='panel'):
                    filterselection = gr.Dropdown(filternames, value="None", label="Colorizer/FilterFX",
                                                  interactive=True)
        ...
        with gr.Row():
            extra_files_output = gr.Files(label='Salida de archivos resultantes', file_count="multiple")

    start_cut_video.click(fn=on_cut_video, inputs=[files_to_process, cut_start_time, cut_end_time, extras_chk_encode],
    ...
    start_join_videos.click(fn=on_join_videos, inputs=[files_to_process, extras_chk_encode],
                            outputs=[extra_files_output])
    extras_create_video.click(fn=on_extras_create_video,
                              inputs=[extras_images_folder, extras_fps, extras_chk_creategif],
                              outputs=[extra_files_output])
    start_frame_process.click(fn=on_frame_process, inputs=[files_to_process, filterselection, upscalerselection],
                              outputs=[extra_files_output])
...
def on_cut_video(files, cut_start_frame, cut_end_frame, reencode):
    ...
    for tf in files:
        ...
        if os.path.isfile(destfile):
            ...
            gr.Error('¡Error al cortar el vídeo!')
    return resultfiles
...
def on_frame_process(files, filterselection, upscaleselection):
    ...
    if roop.globals.CFG.clear_output:
        try:
            ...
            gr.Warning(f'Error al limpiar carpeta de salida: {e}')
    prepare_environment()
