...
def faceswap_tab():
    ...
    with gr.Row(variant="panel"):
            ...
            with gr.Column(scale=2):
                ...
                with gr.Row(visible=True, variant="panel", elem_classes=["dynamic-panel"]) as dynamic_face_selection:
                    with gr.Column(scale=2):
                        # Gallery solo para mostrar caras (sin evento select para evitar bug de Gradio)
                        face_selection = gr.Gallery(
                            elem_id="face_selection_gallery",
                            columns=6,
                            height=250,
                            object_fit="contain",
                            label="Caras detectadas - usa el slider y botón OK para seleccionar",
                            allow_preview=False,
                            preview=False,
                            interactive=False,  # Desactivado para evitar bugs
                        )
                        
                        # Slider para seleccionar cara
                        face_selector_slider = gr.Slider(
                            minimum=1,
                            maximum=1,
                            value=1,
                            step=1,
                            label="Selecciona el número de cara",
                            info="Mueve el slider y confirma con el botón",
                            interactive=True,
                            elem_id="face_selector_slider",
                        )
    ...
    # Slider para seleccionar cara - solo selecciona, no confirma
    ...
    # NOTE: Debido a un bug conocido de Gradio (Issue #13314), el evento .select() en Gallery
    # causa KeyError: 'value' al actualizar dinámicamente. Se deshabilita el click directo.
    # La selección se hace mediante el slider y el botón "Usar cara seleccionada".
    pass
    ...
    # Evento para selección de cara vía HTML personalizado
    selected_face_idx.change(
        fn=on_face_html_selected,
        inputs=[selected_face_idx],
        outputs=[
            face_selection,
            selected_target_text,
    ...
    try:
        ...
        try:
                        ...
                        if len(face_data) >= 2:
                            ...
                            if hasattr(face_obj, "bbox") and len(face_obj.bbox) >= 4:
                                try:
                                    ...
                                    if face_img is not None and face_img.size > 0:
                                        # IMPORTANTE: face_img ya está en RGB desde extract_face_images
                                        # REDIMENSIONAR a tamaño fijo para que el click funcione
                                        import cv2
                                        target_size = (128, 128)  # Tamaño fijo para todas las miniaturas
                                        face_resized = cv2.resize(face_img, target_size, interpolation=cv2.INTER_AREA)
                                        image = util.convert_to_gradio(face_resized, is_rgb=True)
                                        thumbs.append(image)
                                except Exception as e:
            ...
            if len(thumbs) >= 1 and SELECTION_FACES_DATA:
                ...
                return (
                    gr.update(),  # dynamic_face_selection
                    generate_faces_html(thumbs),  # Mostrar las caras detectadas como HTML
...
def on_face_slider_changed(slider_value):
    ...
    return (
        gr.update(),  # dynamic_face_selection (sin cambios)
        generate_faces_html(highlighted_thumbs, selected_idx=face_index),  # face_selection_html - galería con cara resaltada
        gr.update(),  # target_faces (sin cambios)
        gr.update(),  # selected_face_detection (sin cambios)
...
def on_face_html_selected(face_idx=-1):
    """
    Maneja la selección de cara desde el HTML personalizado.
    Recibe el índice desde el componente selected_face_idx.
    """
    global TEMP_SELECTED_FACE_INDEX, SELECTION_FACES_DATA
    
    print(f"[DEBUG] on_face_html_selected - face_idx: {face_idx}")
    
    if face_idx < 0 or SELECTION_FACES_DATA is None or len(SELECTION_FACES_DATA) == 0:
        print(f"[DEBUG] on_face_html_selected - Índice inválido o sin datos")
        return (
            gr.update(),  # face_selection_html
            gr.update(),  # selected_target_text
            gr.update(),  # face_selector_slider
        )
    
    if face_idx >= len(SELECTION_FACES_DATA):
        print(f"[ERROR] on_face_html_selected - Índice fuera de rango: {face_idx}")
        return (
            gr.update(),  # face_selection_html
            gr.update(),  # selected_target_text
            gr.update(),  # face_selector_slider
    ...
    TEMP_SELECTED_FACE_INDEX = face_idx
    print(f"[DEBUG] on_face_html_selected - Índice temporal: {face_idx}")
    
    # Regenerar HTML con la cara seleccionada resaltada
    thumbs = []
    for i in range(len(SELECTION_FACES_DATA)):
        if i < len(SELECTION_FACES_DATA):
            fd = SELECTION_FACES_DATA[i]
            if len(fd) >= 2:
                thumbs.append(fd[1])  # La miniatura está en fd[1]
    
    # Actualizar UI
    return (
        generate_faces_html(thumbs, selected_idx=face_idx),  # face_selection_html actualizado
        gr.update(value=f"**[OK] Cara {face_idx + 1} seleccionada - Haz clic en 'Usar cara seleccionada' para confirmar**"),  # selected_target_text
        gr.update(value=face_idx + 1),  # face_selector_slider (1-based)
    )
...
def on_face_selection_click(evt: gr.SelectData):
    """
    Maneja el click directo en una cara de la galería de selección.
    """
    global TEMP_SELECTED_FACE_INDEX, SELECTION_FACES_DATA
    
    print(f"[DEBUG] on_face_selection_click - evt: {evt is not None}")
    ...
    face_index = evt.index
    print(f"[DEBUG] on_face_selection_click - face_index: {face_index}")
    
    if SELECTION_FACES_DATA is None or len(SELECTION_FACES_DATA) == 0:
        print("[ERROR] on_face_selection_click - No hay SELECTION_FACES_DATA")
        return (
            gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update()
        )
    
    if face_index < 0 or face_index >= len(SELECTION_FACES_DATA):
        print(f"[ERROR] on_face_selection_click - Índice inválido: {face_index}")
        return (
            gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update()
        )
    
    TEMP_SELECTED_FACE_INDEX = face_index
    print(f"[DEBUG] on_face_selection_click - Llamando on_confirm_use_selected_face")
    
    # Llamar a la función de confirmación directamente
    return on_confirm_use_selected_face()
...
def on_selected_face(face_index, detected_faces=None):
    ...
    if IS_INPUT:
        ...
        return (
            gr.update(),  # dynamic_face_selection después de seleccionar
            [],  # Limpiar face_selection
            ui.globals.ui_target_thumbs,  # target_faces - devolver lista completa
