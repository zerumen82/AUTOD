...
def on_generate(img_data, p_text, engine_val, f_preserve, use_ai_val, enhance_val, quality_mode_val, enhance_tier_val, lora_name, lora_strength, denoise_val):
    ...
    if not p_text and not quality_mode_val:
    ...
    try:
        ...
        if res_img:
            ...
            return res_img, f"✅ {msg} - Guardado en {output_dir}", mask_img
        else:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                with gr.Accordion("⚙️ Opciones Avanzadas", open=False):
                    ...
                    engine = gr.Dropdown(
                        choices=[
                            ("✨ Grok Imagine (Turbo) (rápido + calidad, img2img real)", "imagine"),
                            ("FLUX.1 Dev Abliterated (txt2img puro, lento, sin img2img)", "flux_dev_abliterated"),
                            ("LongCat Image Edit (Full, experimental)", "longcat_full"),
