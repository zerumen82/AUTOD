...
def detect_mouth_open(target_face: Face, landmarks_106=None) -> tuple:
    ...
    Usa landmarks de 106 puntos si están disponibles, sino usa los 5 keypoints básicos.
    
    Returns:
        tuple: (is_open: bool, mouth_region: dict with landmarks, open_ratio: float)
    """
    try:
        mouth_open_threshold = 0.15  # Umbral para considerar la boca abierta
        
        # Si tenemos landmarks de 106 puntos, usarlos para análisis preciso
        ...
        # Fallback: usar los 5 keypoints básicos (kps)
            ...
            if len(kps) >= 5:
                ...
                # Heurística simple: si las comisuras están más separadas verticalmente
                # de lo esperado, la boca puede estar abierta
                is_open = vertical_diff > 0.1
...
def create_mouth_preservation_mask(target_img: np.ndarray, mouth_region: dict, 
    ...
    try:
        ...
        # Crear región elíptica alrededor de la boca
        # Más ancha que alta para cubrir toda la boca
        radius_x = int(mouth_width * 0.8) if mouth_width else 30
        radius_y = int(mouth_height * 1.5) if mouth_height else 20
        ...
        blur_size = int(max(5, min(radius_x, radius_y) // 2) | 1)
...
class FaceSwap:
    ...
    def _create_ellipse_mask(shape):
        """Crea una máscara elíptica para la cara como fallback"""
        h, w = shape[:2]
        mask = np.zeros((h, w), dtype=np.float32)
        center = (w // 2, h // 2)
        radius_x = int(w * 0.42)  # Ancho natural
        radius_y = int(h * 0.45)  # Un poco más alto para mejor cobertura de boca
