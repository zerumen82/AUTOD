...
def pre_check() -> bool:
    ...
    util.conditional_download(download_directory_path,
...
def batch_process_regular(files: list[ProcessEntry], masking_engine: str, new_clip_text: str, use_new_method, imagemask,
                          num_swap_steps, progress, selected_index=0) -> None:
    ...
    if len(roop.globals.INPUT_FACESETS) <= selected_index:
        selected_index = 0
    options = ProcessOptions(get_processing_plugins(masking_engine), roop.globals.distance_threshold,
                             roop.globals.blend_ratio, roop.globals.face_swap_mode, selected_index, new_clip_text, mask,
                             num_swap_steps, False)
    process_mgr.initialize(roop.globals.INPUT_FACESETS, roop.globals.TARGET_FACES, options)
...
def batch_process(files: list[ProcessEntry], use_new_method) -> None:
    ...
    for index, f in enumerate(files):
        ...
        if util.has_image_extension(fullname):
            destination = util.get_destfilename_from_path(fullname, roop.globals.output_path,
        ...
        for index, v in enumerate(videofiles):
            ...
            if roop.globals.keep_frames or not use_new_method:
                ...
                process_mgr.run_batch_inmem(v.filename, v.finalname, v.startframe, v.endframe, fps,
                                            roop.globals.execution_threads, skip_audio)
