...
class Settings:
    ...
    def default_get(self, data, name, default):
        ...
        try:
            ...
            # print(f"Error getting {name} from config file")
            pass
        return value
    ...
    def load(self):
        ...
        # Hardware Optimization (8GB VRAM)
        self.low_vram = self.default_get(data, 'low_vram', True)
        self.enable_fp16 = self.default_get(data, 'enable_fp16', True)
        self.batch_processing_size = self.default_get(data, 'batch_processing_size', 4)
        self.max_batch_threads = self.default_get(data, 'max_batch_threads', 4)
        self.cuda_malloc_async = self.default_get(data, 'cuda_malloc_async', True)
        ...
        self.output_path = self.default_get(data, 'output_path', '')
        ...
        # API Tokens for online services
        self.huggingface_token = self.default_get(data, 'huggingface_token', '')
        self.replicate_token = self.default_get(data, 'replicate_token', '')
        self.fal_token = self.default_get(data, 'fal_token', '')
