...
from DEP.utils import check_shape
from DEP.transform.gan.mask import CorrectToMask, MaskrefToMaskdet, MaskfinToNude
from DEP.transform.opencv.correct import DressToCorrect, ColorTransfer
from DEP.transform.opencv.mask import MaskToMaskref, MaskdetToMaskfin
from DEP.transform.opencv.resize import ImageToResized, ImageToCrop, ImageToOverlay, ImageToResizedCrop, ImageToRescale, ImageToNearest, ImageCompress
...
def scale_mod(args, p):
    ...
    # Check if input is a list and iterate over it
    if isinstance(Conf.args["input"], list):
        for input_path in Conf.args["input"]:
            if not os.path.isfile(input_path):
                Conf.log.error(f"Input path {input_path} is not a file.")
                return []  # Return an empty list or handle the error as needed
    else:
        # Single input path, directly check if it's a file
        if not os.path.isfile(Conf.args["input"]):
            Conf.log.error(f"Input path {Conf.args['input']} is not a file.")
            return []  # Return an empty list or handle the error as needed
